...
namespace QLThuPhiDienNuoc.DAL
{
    public class HoaDonDAL
    {
        ...
        public bool ThanhToanHoaDon(string maHo, int thang, int nam)
        {
            string query = "UPDATE HoaDon SET TrangThai = N'Đã nộp', NgayThanhToan = GETDATE() WHERE MaHoDan = @MaHo AND Thang = @Thang AND Nam = @Nam";
            return db.ExecuteNonQuery(query, new object[] { maHo, thang, nam }) > 0;
        }
    }
}
